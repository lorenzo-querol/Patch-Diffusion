...
def random_patch(images, patch_size, resolution):
    ...
    x_pos = (x_pos / resolution - 0.5) * 2.0
    y_pos = (y_pos / resolution - 0.5) * 2.0
...
def ablation_sampler(
    ...
    for i, (t_cur, t_next) in enumerate(zip(t_steps[:-1], t_steps[1:])):  # 0, ..., N-1
        ...
        if solver == "euler" or i == num_steps - 1:
            ...
            d_prime = (sigma_deriv(t_prime) / sigma(t_prime) + s_deriv(t_prime) / s(t_prime)) * x_prime - sigma_deriv(t_prime) * s(t_prime) / sigma(
                t_prime
            ) * denoised
...
def main(
    ...
    **sampler_kwargs,
    ...
    for batch_seeds in tqdm.tqdm(rank_batches, unit="batch", disable=(dist.get_rank() != 0)):
        ...
        if mask_pos:
            latents_pos = torch.zeros_like(latents_pos)
        if embed_fq > 0:
