...
def open_cifar10(tarball: str, *, max_images: Optional[int], is_test: bool):
    images = []
    labels = []
...
def open_cifar10_test(tarball: str, *, max_images: Optional[int]):
    ...
    with tarfile.open(tarball, "r:gz") as tar:
        member = tar.getmember("cifar-10-batches-py/data_batch_5")
        with tar.extractfile(member) as file:
            data = pickle.load(file, encoding="latin1")
        images.append(data["data"].reshape(-1, 3, 32, 32))
        labels.append(data["labels"])
...
def open_dataset(source, *, max_images: Optional[int], is_test: bool = False):
    if os.path.isdir(source):
        ...
        if os.path.basename(source) == "cifar-10-python.tar.gz":
            return open_cifar10(source, max_images=max_images, is_test=is_test)
...
@click.option("--is_test", help="Is test dataset", is_flag=True)
def main(source: str, dest: str, max_images: Optional[int], transform: Optional[str], resolution: Optional[Tuple[int, int]], is_test: bool):
    ...
    num_files, input_iter = open_dataset(source, max_images=max_images, is_test=is_test)
