...
from calibration import accepts, hpdi, squeeze_generic
...
class _Miscalibration(object):
    ...
    def __init__(self, bins: Union[int, Iterable[int]] = 10, equal_intervals: bool = True, detection: bool = False, sample_threshold: int = 1):
    ...
    def frequency(
        self, X: Union[Iterable[np.ndarray], np.ndarray], y: Union[Iterable[np.ndarray], np.ndarray], batched: bool = False, uncertainty: str = "mean"
    ...
    def reduce(self, histogram: np.ndarray, distribution: np.ndarray, axis: int, reduce_result: Tuple = None):
        ...
        if reduce_result is None:
            ...
            extended_hist = np.repeat(np.expand_dims(samples_map, axis=axis), distribution.shape[axis], axis=axis)
            ...
            rel_samples_hist_reduced_conf = np.divide(distribution, extended_hist, out=np.zeros_like(distribution), where=extended_hist != 0)
        else:
    ...
    def prepare(
        self, X: Union[Iterable[np.ndarray], np.ndarray], y: Union[Iterable[np.ndarray], np.ndarray], batched: bool = False, uncertainty: str = None
    ...
    def process(
        self, metric: str, acc_hist: np.ndarray, conf_hist: np.ndarray, variance_hist: np.ndarray, num_samples_hist: np.ndarray
        ...
        if metric == "ace":
            ...
            bin_map = np.divide(reduced_deviation_map, non_empty_bins, out=np.zeros_like(reduced_deviation_map), where=non_empty_bins != 0)
    ...
    def _prepare_bins(self, X: List[np.ndarray], num_features: int, min_: Union[float, List[float]] = 0.0, max_: Union[float, List[float]] = 1.0) -> List[List[np.ndarray]]:
    ...
    def _prepare_uncertainty(self, X: np.ndarray, y: np.ndarray, uncertainty: str) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        ...
        if uncertainty in ["mean", "median", "mode"]:
            ...
            if (y.ndim == 2 and self.detection) or (y.ndim == 2 and X.ndim == 2 and not self.detection) or (y.ndim == 3 and X.ndim == 3 and not self.detection):
                raise ValueError("Separate ground-truth information is provided for each probability forward pass " "but uncertainty type 'mean', 'median' or 'mode' is specified.")
