...
def mv_cauchy_log_density(value: torch.Tensor, loc: torch.Tensor, cov: torch.Tensor):
    ...
    const = torch.lgamma((n_dims + 1) / 2.0) - torch.lgamma(torch.tensor(0.5)) - (n_dims / 2) * torch.log(torch.tensor(np.pi))  # scalar

    logdet = 0.5 * torch.logdet(cov)  # ([r], n)
    mahalanobis = torch.transpose(diff, dim0=-2, dim1=-1) @ invcov @ diff  # ([r], n, 1, 1)

    log_density = const - logdet - ((n_dims + 1) / 2.0) * torch.log(1.0 + mahalanobis[..., 0, 0])  # ([r], n)
