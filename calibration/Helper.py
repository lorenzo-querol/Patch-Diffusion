...
def _extract_input(X: Iterable[np.ndarray]) -> Tuple[np.ndarray, np.ndarray, bool]:
    ...
    assert len(X) == 2, "If 'X' is provided with type of {list, tuple}, this methods asserts a length of 2 " "with X[0] as the predicted mean and X[1] as the predicted stddev."
...
def meanvar(X: Union[Iterable[np.ndarray], np.ndarray], y: np.ndarray = None) -> Union[Tuple[np.ndarray, np.ndarray, bool], Tuple[Tuple[np.ndarray, np.ndarray], np.ndarray, bool]]:
    """
    ...
    if isinstance(X, (tuple, list)):
        ...
        assert X.ndim >= 2, (
            "If 'X' is provided as NumPy array, this methods asserts values obtained by "
        ...
        raise ValueError(
            "Parameter 'X' must be either of type {list, tuple} with mean and stddev passed or "
...
def is_in_quantile(
    X: Union[Iterable[np.ndarray], np.ndarray], y: np.ndarray, q: Union[float, Iterable[float], np.ndarray], kind: str
    ...
    if kind == "cumulative":
        ...
        nearest_min = np.argmin(np.abs(cdf[None, ...] - (alpha / 2.0)), axis=1)  # (q, t, n, d)  # (q, n, d)

        nearest_max = np.argmin(np.abs(cdf[None, ...] - (1.0 - (alpha / 2.0))), axis=1)  # (q, t, n, d)  # (q, n, d)
        ...
        qbounds_next = np.where(np.abs(qbounds_min - y[None, ...]) < np.abs(qbounds_max - y[None, ...]), qbounds_min, qbounds_max)  # (q, n, d)
    ...
    # Cauchy distribution is given - check if samples are within two-sided quantile boundaries
        ...
        qbounds_min = cauchy.ppf(alpha / 2.0, loc=mode[None, ...], scale=scale[None, ...])  # (q, n, d)
        qbounds_max = cauchy.ppf(1.0 - (alpha / 2.0), loc=mode[None, ...], scale=scale[None, ...])  # (q, n, d)
        ...
        if isinstance(X, (tuple, list)):
            ...
            # in the independent case, we also use the Mahalanobis distance but for each dimension separately
                ...
                qbounds_min = norm.ppf(np.expand_dims(alpha / 2.0, axis=(1, 2)), loc=mean[None, ...], scale=stddev[None, ...])  # (q, 1, 1)  # (1, n, d)  # (1, n, d)  # (q, n, d)

                qbounds_max = norm.ppf(np.expand_dims(1.0 - (alpha / 2.0), axis=(1, 2)), loc=mean[None, ...], scale=stddev[None, ...])  # (q, 1, 1)  # (1, n, d)  # (1, n, d)  # (q, n, d)

                # now choose the quantile boundaries that are closer to our ground-truth Y
                qbounds_next = np.where(np.abs(qbounds_min - y[None, ...]) < np.abs(qbounds_max - y[None, ...]), qbounds_min, qbounds_max)
